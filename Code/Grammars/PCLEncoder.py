...
class PCLEncoder(PiCalcListener):
	...
	##   When a branch is entered, the channel in the branch is given a backup encoding in encFunc
	...
	def checkBranchStack(self, channel):
		if self.branchStack != []:
			if self.branchStack[-1] != 'B':
				...
				channelBranchBackup = channel + u"·Branch"
				if channelBranchBackup in self.encFunc:
					self.encFunc[channel] = self.encFunc[channelBranchBackup]
					self.branchStack.append('C')
	...
	## This function has a bug if multiple branches use the channel in the original branch statement in their continuation processes
	## e.g. branch(x){ li: send(x, ...), lj: send(x, ...) ... }
	## When this occurs, each branch replaces x with the channel generated by the previous continuations encoding, 
	## rather than the channel generated by the branch statements encoding
	## e.g. li_(c) > (new d) send(c, ..., d), lj_(c) > (new e) send(d, ..., e)
	## This should be ... (new e) send(c, ..., e)
	def enterBranching(self, ctx):
		...
		for i in range(len(ctx.option)):
			...
			if i != (len(ctx.option) - 1):
				...
				brnStrBuilder = brnStrBuilder + ' }'
		self.encFunc[ctx.channel.getText()] = newChan
		channelBranchBackup = ctx.channel.getText() + u"·Branch"
		self.encFunc[channelBranchBackup] = newChan
